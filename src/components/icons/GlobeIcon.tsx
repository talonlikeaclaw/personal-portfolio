...
export default function GlobeIcon({ size = 24, color = "currentColor", strokeWidth = 2, className = "" }: AnimatedIconProps) {
  ...
  const start = useCallback(() => { if (!reducedMotion) animate(".globe-circle", { rotate: 360 }, { duration: 0.9, ease: "easeInOut" }); }, [animate, reducedMotion]);
  const stop = useCallback(() => animate(".globe-circle", { rotate: 0 }, { duration: 0.2 }), [animate]);
